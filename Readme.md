...
##<a name="5-level"></a>流水线技术的基本概念及ARM9采用的5级流水线:
...
重点理解并记忆：+	[寄存器结构及组织](#jcqjg)+	[处理器的运行模式](#clqyx)+	[处理器的工作状态](#clqgz)+	[处理器的寄存器组织](#clqjcq)+	[异常处理的类型及含义、异常向量表、优先级](#yccl)+	***异常的响应过程及返回过程***+	[存储器结构(地址空间、存储器格式数据类型(在P21)等)。](#ccqjg)
##<a name="jcqjg"></a>寄存器结构
ARM微处理器共有37个寄存器，被分成若干个组，包括：

+ 31个通用寄存器，包括程序计数器，均为32位寄存器。
+ 6个状态寄存器，用于标示CPU的工作状态及程序的运行状态，均为32位。目前只使用了一部分。##<a name="jcqzz"></a>寄存器组织
...
##<a name="yccl"></a>异常处理的类型及含义、异常向量表、优先级
...
##<a name="ccqjg"></a>存储器结构(地址空间、存储器格式、数据类型等)。
...
#<a name="c5"></a>第五章32位RISC微处理器S3C2410A通读3.1节、3.2节、3.3节、3.4节及3.5节。
...
+ 时钟 电源管理
	+ 片上MPLL和UPLL（PLL:phase locked loop,锁相环）
		+ UPLL产生用于USB主机／设备操作的时钟。
		+ MPLL产生操作MCU（Microcontroller Unit）的时钟，可达266MHz。
	+ 电源模式
		+ 正常模式位正常运行模式
		+ 慢速模式位不加锁相环(PLL)的低时钟频率模式
		+ 空闲模式只停止CPU的时钟
		+ 掉电模式切断所有外设和内核的电源
	## 存储器控制器## 时钟电路## 电源电路## I/O口及相关寄存器#<a name="c6"></a>第六章 嵌入式系统的存储器系统通读4.1.1节、4.2节、4.4节。
重点理解并记忆：+ 嵌入式系统存储设备层次结构+ 存储器部件的分类+ 常见的嵌入式系统存储器（RAM、ROM、Flash Memory）+ ***NAND Flash控制器特性及控制器结构（自动启动模式的时序）。***
## 嵌入式系统存储设备层次结构## 存储器部件的分类## 常见的嵌入式系统存储器（RAM、ROM、Flash Memory）
#<a name="c8"></a>第八章 嵌入式系统总线接口通读6.1节、6.2.1节讲授内容重点理解并记忆：+ 异步通信方式+ RS-232C接口信号线RXD和TXD+ UART的概念、传送数据的格式、UART结构+ I2C接口基本原理与结构。
